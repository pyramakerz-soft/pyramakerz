    ...
    <section id="contact" class="contact">
        <div class="container">
            <div class="section-header" data-aos="zoom-in-up" data-aos-delay="200">
                <h2>{{ __('admin/add_package.edit') }} </h2>
            </div>
        </div>
                    ...
                    <form id="package-form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="en_name" class="form-control" id="en_name"
                                    placeholder="{{ __('admin/add_package.name_en') }}" value="{{ $package->name }}" required>
                            </div>
                            <div class="col-md-6 form-group mt-3 mt-md-0">
                                <label>{{ __('admin/add_package.name_ar') }}:</label>
                                <input type="text" class="form-control" name="ar_name" id="ar_name"
                                    placeholder="{{ __('admin/add_package.name_ar') }}" value="{{ $package->ar_name }}" required>
                            </div>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_package.desc') }}:</label>
                            <input type="text" class="form-control" name="description" id="description"
                                placeholder="{{ __('admin/add_package.desc') }}" value="{{ $package->description }}" required>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_package.desc_ar') }}:</label>
                            <input type="text" class="form-control" name="ar_description" id="ar_description"
                                placeholder="{{ __('admin/add_package.desc_ar') }}" value="{{ $package->ar_description }}" required>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_package.bullet_points') }}:</label>
                            <div id="bullet-container">
                                @foreach($package->bullets ?? [] as $index => $bullet)
                                <div class="input-group mb-2">
                                    <input type="text" name="bullets[]" class="form-control" value="{{ $bullet }}" placeholder="{{ __('admin/add_package.bullet_points') }}">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <div id="bullet-container-ar">
                                @foreach($package->ar_bullets ?? [] as $index => $bullet)
                                <div class="input-group-ar mb-2">
                                    <input type="text" name="ar_bullets[]" class="form-control" value="{{ $bullet }}" placeholder="{{ __('admin/add_package.bullet_points_ar') }}">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <input type="number" class="form-control" name="price" id="price"
                                placeholder="{{ __('admin/add_package.price') }}" required step="0.01" value="{{ $package->price }}"
                                min="0">
                        </div>
                        <div class="form-group mt-3">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <div id="product-selection">
                                ...
                                @php
                                $attachedProduct = $package->products->firstWhere('id', $product->id);
                                @endphp
                                <div class="d-flex align-items-center my-2">
                                    <input
                                        ...
                                        {{ $attachedProduct ? 'checked' : '' }}>

                                    <span class="mx-2" style="color: black; font-size: 16px;">
                                        {{ $product->name }}
                                    </span>

                                    <input
                                        type="number"
                                        class="form-control product-quantity {{ $attachedProduct ? '' : 'd-none' }}"
                                        placeholder="Quantity"
                                        min="1"
                                        value="{{ $attachedProduct ? $attachedProduct->pivot->quantity : 1 }}"
                                        style="width: 80px;">
                                </div>
                                @endforeach
                            </div>
                        ...
                        <div class="text-center mt-3 d-flex justify-content-center">
                            <button type="submit" class="btn btn-sucess" id="submitForm" data-aos="flip-up"
                                data-aos-delay="200">{{ __('admin/add_package.edit') }}</button>
                        </div>
                    </form>
                </div>
...
<script>
    document.getElementById("package-form").addEventListener("submit", function(e) {
        ...
        axios.post(@json(url('/api/packages/edit')), formData, {
                headers: {
                    "Authorization": `Bearer ${token}`,
            ...
            .then(response => {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('admin.viewPackages') }}";
                }, 2000);
                submitBtn.disabled = false;
            })
            .catch(error => {
                let message = "Failed to edit package.";
