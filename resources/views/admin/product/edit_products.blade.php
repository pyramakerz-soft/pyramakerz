    ...
    <section id="contact" class="contact" data-aos="zoom-in" data-aos-delay="200">
        <div class="container">
            ...
            <div class="section-header">
                <h2>{{ __('admin/add_product.edit') }} </h2>
            </div>
        </div>
                    ...
                    <form id="product-form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="en_name" class="form-control" id="en_name"
                                    placeholder="{{ __('admin/add_product.name_en') }}" value="{{ $product->name }}" required>
                            </div>
                            <div class="col-md-6 form-group mt-3 mt-md-0">
                                <label>{{ __('admin/add_product.name_ar') }}:</label>
                                <input type="text" class="form-control" name="ar_name" id="ar_name"
                                    placeholder="{{ __('admin/add_product.name_ar') }}" value="{{ $product->ar_name }}" required>
                            </div>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_product.desc') }}:</label>
                            <input type="text" class="form-control" name="description" id="description"
                                placeholder="{{ __('admin/add_product.desc') }}" value="{{ $product->description }}" required>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_product.desc_ar') }}:</label>
                            <input type="text" class="form-control" name="ar_description" id="ar_description"
                                placeholder="{{ __('admin/add_product.desc_ar') }}" value="{{ $product->ar_description }}" required>
                        </div>
                        <div class="form-group mt-3">
                            <label>{{ __('admin/add_product.price') }}:</label>
                            <input type="number" class="form-control" name="price" id="price"
                                placeholder="{{ __('admin/add_product.price') }}" required step="0.01" value="{{ $product->price }}"
                                min="0">
                        </div>
                        <div class="form-group mt-3">
                        ...
                        <div class="text-center mt-3 d-flex justify-content-center">
                            <button type="submit" class="btn btn-sucess" id="submitForm" data-aos="flip-up"
                                data-aos-delay="200">{{ __('admin/add_product.edit') }}</button>
                        </div>
                    </form>
                </div>
...
<script>
    document.getElementById("product-form").addEventListener("submit", function(e) {
        ...
        axios.post(@json(url('api/products/edit')), formData, {
                headers: {
                    "Authorization": `Bearer ${token}`,
            ...
            .then(response => {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('admin.viewProducts') }}";
                }, 2000);
                submitBtn.disabled = false;
            })
            .catch(error => {
                let message = "Failed to edit product.";
